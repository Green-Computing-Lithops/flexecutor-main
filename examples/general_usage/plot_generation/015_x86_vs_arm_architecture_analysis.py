...
def find_default_analysis_files():
    """Find default x86 and ARM analysis files from the 001_analysis_results directory."""
    analysis_dir = Path("001_analysis_results")
    
    if not analysis_dir.exists():
        return None, None
    
    # Look for representative files (prefer ml workload with 2048MB memory)
    x86_candidates = list(analysis_dir.glob("*x86_analysis.json"))
    arm_candidates = list(analysis_dir.glob("*arm_analysis.json"))
    
    # Prefer ml stage workloads with 2048MB memory
    x86_preferred = [f for f in x86_candidates if "ml_stage" in f.name and "2048" in f.name]
    arm_preferred = [f for f in arm_candidates if "ml_stage" in f.name and "2048" in f.name]
    
    # If no ml files, try other workloads
    if not x86_preferred:
        x86_preferred = [f for f in x86_candidates if "2048" in f.name]
    if not arm_preferred:
        arm_preferred = [f for f in arm_candidates if "2048" in f.name]
    
    # Fallback to any available files
    x86_file = x86_preferred[0] if x86_preferred else (x86_candidates[0] if x86_candidates else None)
    arm_file = arm_preferred[0] if arm_preferred else (arm_candidates[0] if arm_candidates else None)
    
    return str(x86_file) if x86_file else None, str(arm_file) if arm_file else None
...
def main():
    ...
    # Find default files
    default_x86_file, default_arm_file = find_default_analysis_files()
    
    parser.add_argument('--x86-file', default=default_x86_file,
                       help=f'Path to x86 analysis JSON file (default: {default_x86_file})')
    parser.add_argument('--arm-file', default=default_arm_file,
                       help=f'Path to ARM analysis JSON file (default: {default_arm_file})')
    parser.add_argument('--output-dir', default='015_x86_vs_arm_architecture_analysis', 
                       help='Output directory for generated analysis')
    
    args = parser.parse_args()
    
    # Check if default files were found and provided
    if not args.x86_file:
        print("❌ Error: No x86 analysis file found. Please specify --x86-file")
        print("   Available files should be in 001_analysis_results/ directory")
        return
    
    if not args.arm_file:
        print("❌ Error: No ARM analysis file found. Please specify --arm-file")
        print("   Available files should be in 001_analysis_results/ directory")
        return
    
    if not Path(args.x86_file).exists():
        print(f"❌ Error: x86 file not found: {args.x86_file}")
        return
    
    if not Path(args.arm_file).exists():
        print(f"❌ Error: ARM file not found: {args.arm_file}")
        return
    
    print("🔍 Loading analysis data...")
    print(f"📄 Using x86 file: {args.x86_file}")
    print(f"📄 Using ARM file: {args.arm_file}")
