#!/usr/bin/env python3
"""
Universal RAPL Energy Visualization - Combined Analysis

This script generates comprehensive RAPL energy plots for all example types
...
Usage:
    python generate_universal_rapl_energy_plots.py

Output:
    PNG files with comprehensive RAPL energy visualization for each configuration
"""
...
# Professional color palette for RAPL metrics
RAPL_COLORS = {
    'rapl_energy_pkg': '#1f77b4',      # Professional blue for package
    'rapl_energy_cores': '#ff7f0e',    # Professional orange for cores
    'rapl_energy_total': '#2ca02c',    # Professional green for total
    'rapl_efficiency': '#d62728',      # Professional red for efficiency
...
def extract_rapl_data(analysis_data, filename):
    """Extract RAPL energy data from analysis results."""
    ...
    rapl_metrics = ['rapl_energy_pkg', 'rapl_energy_cores', 'rapl_energy_total']
    for metric in rapl_metrics:
        if metric in metrics:
            rapl_data[metric] = metrics[metric]
        else:
            rapl_data[metric] = {'mean': 0, 'min': 0, 'max': 0, 'std_dev': 0, 'count': 0, 'total': 0}
    
    # Extract efficiency data
    if 'rapl_energy_efficiency' in derived_metrics:
        rapl_data['efficiency'] = derived_metrics['rapl_energy_efficiency']
    else:
        rapl_data['efficiency'] = {'joules_per_second': 0, 'description': 'N/A'}
    
    # Extract timing data for context
    if 'compute' in metrics:
        rapl_data['compute_time'] = metrics['compute']
    else:
        rapl_data['compute_time'] = {'mean': 0, 'min': 0, 'max': 0, 'std_dev': 0}
...
def process_analysis_files(analysis_dir):
    ...
    for file_path in files:
        ...
        rapl_data = extract_rapl_data(analysis_data, filename)
        if rapl_data is None:
            continue
            
        # Create group key: example_type_stage_memory
        group_key = f"{rapl_data['example_type']}_{rapl_data['stage']}_{rapl_data['memory']}"
        if group_key not in example_groups:
            example_groups[group_key] = []
        
        example_groups[group_key].append(rapl_data)
...
def create_combined_rapl_energy_plots(example_groups, output_dir):
    """Create combined RAPL energy plots for each example type, stage, and memory configuration."""
    setup_plot_style()
    ...
    for group_key, data_list in example_groups.items():
        ...
        pkg_means = [d['rapl_energy_pkg']['mean'] for d in data_list]
        pkg_stds = [d['rapl_energy_pkg']['std_dev'] for d in data_list]
        cores_means = [d['rapl_energy_cores']['mean'] for d in data_list]
        cores_stds = [d['rapl_energy_cores']['std_dev'] for d in data_list]
        total_means = [d['rapl_energy_total']['mean'] for d in data_list]
        total_stds = [d['rapl_energy_total']['std_dev'] for d in data_list]
        efficiencies = [d['efficiency']['joules_per_second'] for d in data_list]
        
        # Main plot: RAPL Energy Components vs Workers
        ...
        ax1.set_title(f'RAPL Energy Analysis - {example_name} {stage.title()} (Memory: {memory}MB)', 
        ...
        ax2.plot(workers, efficiencies, marker='o', linewidth=3, markersize=8, 
                color=RAPL_COLORS['rapl_efficiency'], markerfacecolor='white', 
                markeredgewidth=2, markeredgecolor=RAPL_COLORS['rapl_efficiency'])
        ...
        pkg_ranges = [[d['rapl_energy_pkg']['min'], d['rapl_energy_pkg']['max']] for d in data_list]
        cores_ranges = [[d['rapl_energy_cores']['min'], d['rapl_energy_cores']['max']] for d in data_list]
        total_ranges = [[d['rapl_energy_total']['min'], d['rapl_energy_total']['max']] for d in data_list]
        ...
        ax3.errorbar(workers, pkg_means, 
                    ...
                    fmt='o', capsize=5, capthick=2, color=RAPL_COLORS['rapl_energy_pkg'], 
        ...
        ax3.errorbar(workers, cores_means, 
                    ...
                    fmt='s', capsize=5, capthick=2, color=RAPL_COLORS['rapl_energy_cores'], 
        ...
        ax3.errorbar(workers, total_means, 
                    ...
                    fmt='^', capsize=5, capthick=2, color=RAPL_COLORS['rapl_energy_total'], 
        ...
        # Add overall title
        fig.suptitle(f'RAPL Energy Analysis - {example_name} {stage.title()} (Memory: {memory}MB) - All Worker Configurations', 
        ...
        # Save plot
        output_path = os.path.join(output_dir, f'{example_type}_{stage}_{memory}mb_all_workers_rapl_energy.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight', 
                   facecolor='white', edgecolor='none', format='png')
        plt.close()
        
        generated_plots.append(output_path)
        print(f"✅ Generated combined RAPL plot for {example_name} {stage.title()} {memory}MB: {os.path.basename(output_path)}")
    
    return generated_plots

def create_combined_rapl_timeline_plots(example_groups, output_dir):
    ...
    for group_key, data_list in example_groups.items():
        ...
        ax1.plot(workers, pkg_means, marker='o', linewidth=3, markersize=8, 
                color=RAPL_COLORS['rapl_energy_pkg'], label='Package Energy', 
                markerfacecolor='white', markeredgewidth=2)
        ax1.plot(workers, cores_means, marker='s', linewidth=3, markersize=8, 
                color=RAPL_COLORS['rapl_energy_cores'], label='Cores Energy',
                markerfacecolor='white', markeredgewidth=2)
        ax1.plot(workers, total_means, marker='^', linewidth=3, markersize=8, 
                color=RAPL_COLORS['rapl_energy_total'], label='Total Energy',
                markerfacecolor='white', markeredgewidth=2)
        ...
        ax1.set_title(f'RAPL Energy Timeline - {example_name} {stage.title()} (Memory: {memory}MB)', fontweight='bold')
        ax1.legend(loc='upper left')
        ...
        line1 = ax2.plot(workers, efficiencies, marker='D', linewidth=3, markersize=8, 
                        color=RAPL_COLORS['rapl_efficiency'], label='Energy Efficiency (J/s)',
                        markerfacecolor='white', markeredgewidth=2)
        ...
        ax2.set_xlabel('Number of Workers', fontweight='bold')
        ax2.set_ylabel('Energy Efficiency (J/s)', fontweight='bold', color=RAPL_COLORS['rapl_efficiency'])
        ...
        ax2.grid(True, alpha=0.3)
        ax2.set_xticks(workers)
        ax2.tick_params(axis='y', labelcolor=RAPL_COLORS['rapl_efficiency'])
        ...
        # Save plot
        output_path = os.path.join(output_dir, f'{example_type}_{stage}_{memory}mb_all_workers_rapl_timeline.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight', 
                   facecolor='white', edgecolor='none', format='png')
        plt.close()
        
        generated_plots.append(output_path)
        print(f"✅ Generated combined RAPL timeline for {example_name} {stage.title()} {memory}MB: {os.path.basename(output_path)}")
    
    return generated_plots

def main():
    """Main function."""
    print("🚀 Starting Universal RAPL Energy Combined Visualization...")
    print("=" * 80)
    ...
    output_dir = os.path.join(script_dir, '105_local_generate_universal_rapl_energy_plots')
    os.makedirs(output_dir, exist_ok=True)
    
    # Generate combined energy plots
    print(f"\n📊 Generating combined RAPL energy plots...")
    energy_plots = create_combined_rapl_energy_plots(example_groups, output_dir)
    
    # Generate combined timeline plots
    print(f"\n📈 Generating combined RAPL timeline plots...")
    timeline_plots = create_combined_rapl_timeline_plots(example_groups, output_dir)
    
    total_plots = len(energy_plots) + len(timeline_plots)
    
    print(f"\n✅ RAPL energy visualization complete!")
    print(f"📁 Plots saved in: {output_dir}")
    print(f"📊 Generated {total_plots} combined plots ({len(energy_plots)} energy + {len(timeline_plots)} timeline)")
    print(f"🔋 Each plot shows RAPL energy analysis across all worker configurations")
    ...
    for group_key, data_list in example_groups.items():
        ...
        worker_counts = [d['workers'] for d in data_list]
        avg_pkg = statistics.mean([d['rapl_energy_pkg']['mean'] for d in data_list])
        avg_cores = statistics.mean([d['rapl_energy_cores']['mean'] for d in data_list])
        avg_total = statistics.mean([d['rapl_energy_total']['mean'] for d in data_list])
