#!/usr/bin/env python3
"""
Universal Perf Energy Visualization - Combined Analysis

This script generates comprehensive Perf energy plots for all example types
...
Usage:
    python generate_montecarlo_pi_perf_energy_plots.py

Output:
    PNG files with comprehensive Perf energy visualization for each configuration
"""
...
# Professional color palette for Perf metrics
PERF_COLORS = {
    'perf_energy_pkg': '#8e44ad',      # Professional purple for package
    'perf_energy_cores': '#e67e22',    # Professional orange for cores
    'perf_energy_total': '#27ae60',    # Professional green for total
    'perf_efficiency': '#c0392b',      # Professional red for efficiency
...
def extract_perf_data(analysis_data, filename):
    """Extract Perf energy data from analysis results."""
    ...
    perf_data = {
        ...
        'perf_source': metadata.get('perf_source', 'Unknown'),
        'perf_available': metadata.get('perf_available', False),
    }
    
    # Extract Perf energy metrics
    perf_metrics = ['perf_energy_pkg', 'perf_energy_cores', 'perf_energy_total']
    for metric in perf_metrics:
        if metric in metrics:
            perf_data[metric] = metrics[metric]
        else:
            perf_data[metric] = {'mean': 0, 'min': 0, 'max': 0, 'std_dev': 0, 'count': 0, 'total': 0}
    
    # Extract efficiency data
    if 'perf_energy_efficiency' in derived_metrics:
        perf_data['efficiency'] = derived_metrics['perf_energy_efficiency']
    else:
        perf_data['efficiency'] = {'joules_per_second': 0, 'description': 'N/A'}
    
    # Extract timing data for context
    if 'compute' in metrics:
        perf_data['compute_time'] = metrics['compute']
    else:
        perf_data['compute_time'] = {'mean': 0, 'min': 0, 'max': 0, 'std_dev': 0}
...
def process_analysis_files(analysis_dir):
    ...
    for file_path in files:
        ...
        perf_data = extract_perf_data(analysis_data, filename)
        if perf_data is None:
            continue
            
        # Create group key: example_type_stage_memory
        group_key = f"{perf_data['example_type']}_{perf_data['stage']}_{perf_data['memory']}"
        if group_key not in example_groups:
            example_groups[group_key] = []
        
        example_groups[group_key].append(perf_data)
...
def create_combined_perf_energy_plots(example_groups, output_dir):
    """Create combined Perf energy plots for each example type, stage, and memory configuration."""
    setup_plot_style()
    ...
    for group_key, data_list in example_groups.items():
        ...
        pkg_means = [d['perf_energy_pkg']['mean'] for d in data_list]
        pkg_stds = [d['perf_energy_pkg']['std_dev'] for d in data_list]
        cores_means = [d['perf_energy_cores']['mean'] for d in data_list]
        cores_stds = [d['perf_energy_cores']['std_dev'] for d in data_list]
        total_means = [d['perf_energy_total']['mean'] for d in data_list]
        total_stds = [d['perf_energy_total']['std_dev'] for d in data_list]
        efficiencies = [d['efficiency']['joules_per_second'] for d in data_list]
        
        # Main plot: Perf Energy Components vs Workers
        ...
        ax1.set_title(f'Perf Energy Analysis - {example_name} {stage.title()} (Memory: {memory}MB)', 
        ...
        ax2.plot(workers, efficiencies, marker='o', linewidth=3, markersize=8, 
                color=PERF_COLORS['perf_efficiency'], markerfacecolor='white', 
                markeredgewidth=2, markeredgecolor=PERF_COLORS['perf_efficiency'])
        ...
        pkg_ranges = [[d['perf_energy_pkg']['min'], d['perf_energy_pkg']['max']] for d in data_list]
        cores_ranges = [[d['perf_energy_cores']['min'], d['perf_energy_cores']['max']] for d in data_list]
        total_ranges = [[d['perf_energy_total']['min'], d['perf_energy_total']['max']] for d in data_list]
        ...
        ax3.errorbar(workers, pkg_means, 
                    ...
                    fmt='o', capsize=5, capthick=2, color=PERF_COLORS['perf_energy_pkg'], 
        ...
        ax3.errorbar(workers, cores_means, 
                    ...
                    fmt='s', capsize=5, capthick=2, color=PERF_COLORS['perf_energy_cores'], 
        ...
        ax3.errorbar(workers, total_means, 
                    ...
                    fmt='^', capsize=5, capthick=2, color=PERF_COLORS['perf_energy_total'], 
        ...
        config_text += f"Perf Source: {sample_data['perf_source']}\n"
        ...
        # Add overall title
        fig.suptitle(f'Perf Energy Analysis - {example_name} {stage.title()} (Memory: {memory}MB) - All Worker Configurations', 
        ...
        # Save plot
        output_path = os.path.join(output_dir, f'{example_type}_{stage}_{memory}mb_all_workers_perf_energy.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight', 
                   facecolor='white', edgecolor='none', format='png')
        plt.close()
        
        generated_plots.append(output_path)
        print(f"✅ Generated combined Perf plot for {example_name} {stage.title()} {memory}MB: {os.path.basename(output_path)}")
    
    return generated_plots

def create_combined_perf_timeline_plots(example_groups, output_dir):
    ...
    for group_key, data_list in example_groups.items():
        ...
        ax1.set_title(f'Perf Energy Timeline - {example_name} {stage.title()} (Memory: {memory}MB)', fontweight='bold')
        ax1.legend(loc='upper left')
        ax1.grid(True, alpha=0.3)
        ax1.set_xticks(workers)
        
        # Bottom plot: Efficiency and compute time
        ax2_twin = ax2.twinx()
        
        line1 = ax2.plot(workers, efficiencies, marker='D', linewidth=3, markersize=8, 
                        color=PERF_COLORS['perf_efficiency'], label='Energy Efficiency (J/s)',
                        markerfacecolor='white', markeredgewidth=2)
        line2 = ax2_twin.plot(workers, compute_times, marker='*', linewidth=3, markersize=10, 
                             color='#34495e', label='Compute Time (s)',
                             markerfacecolor='white', markeredgewidth=2)
        
        ax2.set_xlabel('Number of Workers', fontweight='bold')
        ax2.set_ylabel('Energy Efficiency (J/s)', fontweight='bold', color=PERF_COLORS['perf_efficiency'])
        ax2_twin.set_ylabel('Compute Time (s)', fontweight='bold', color='#34495e')
        ...
        ax2.grid(True, alpha=0.3)
        ax2.set_xticks(workers)
        ax2.tick_params(axis='y', labelcolor=PERF_COLORS['perf_efficiency'])
        ax2_twin.tick_params(axis='y', labelcolor='#34495e')
        ...
        config_text += f"Perf Source: {sample_data['perf_source']} | Available: {sample_data['perf_available']}"
        ...
        # Save plot
        output_path = os.path.join(output_dir, f'{example_type}_{stage}_{memory}mb_all_workers_perf_timeline.png')
        plt.savefig(output_path, dpi=300, bbox_inches='tight', 
                   facecolor='white', edgecolor='none', format='png')
        plt.close()
        
        generated_plots.append(output_path)
        print(f"✅ Generated combined Perf timeline for {example_name} {stage.title()} {memory}MB: {os.path.basename(output_path)}")
    
    return generated_plots

def main():
    """Main function."""
    print("🚀 Starting Universal Perf Energy Combined Visualization...")
    print("=" * 80)
    ...
    output_dir = os.path.join(script_dir, '104_local_generate_universal_perf_energy_plots')
    os.makedirs(output_dir, exist_ok=True)
    
    # Generate combined energy plots
    print(f"\n📊 Generating combined Perf energy plots...")
    energy_plots = create_combined_perf_energy_plots(example_groups, output_dir)
    
    # Generate combined timeline plots
    print(f"\n📈 Generating combined Perf timeline plots...")
    timeline_plots = create_combined_perf_timeline_plots(example_groups, output_dir)
    
    total_plots = len(energy_plots) + len(timeline_plots)
    
    print(f"\n✅ Perf energy visualization complete!")
    print(f"📁 Plots saved in: {output_dir}")
    print(f"📊 Generated {total_plots} combined plots ({len(energy_plots)} energy + {len(timeline_plots)} timeline)")
    print(f"🔋 Each plot shows Perf energy analysis across all worker configurations")
    ...
    for group_key, data_list in example_groups.items():
        ...
        worker_counts = [d['workers'] for d in data_list]
        avg_pkg = statistics.mean([d['perf_energy_pkg']['mean'] for d in data_list])
        avg_cores = statistics.mean([d['perf_energy_cores']['mean'] for d in data_list])
        avg_total = statistics.mean([d['perf_energy_total']['mean'] for d in data_list])
