"""
run_video_workflow_with_cleanup_batch_execution.py

Runs the video processing workflow with multiple worker configurations (8, 16, 32) sequentially,
...
VIDEO_SCRIPT_PATH = "/home/users/iarriazu/flexecutor-main/examples/video/main_batch_execution.py"
# ───────────────────────────────────────────────────────────────────────────────

def run_video_workflow_with_workers(worker_count):
    """Run the video processing workflow with a specific number of workers by importing and calling the function."""
    print(f"[+] Starting video processing workflow execution with {worker_count} workers...")
    ...
    try:
        ...
        video_dir = "/home/users/iarriazu/flexecutor-main/examples/video"
        ...
        spec = importlib.util.spec_from_file_location("main_batch_execution", 
                                                     "/home/users/iarriazu/flexecutor-main/examples/video/main_batch_execution.py")
        main_batch_module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(main_batch_module)
        run_workflow = main_batch_module.run_video_workflow_with_workers
        
        # Run the workflow with the specified worker count
        success = run_workflow(worker_count)
        
        if success:
            print(f"[✓] Video processing workflow with {worker_count} workers completed successfully")
            return True
        else:
            print(f"[✗] Video processing workflow with {worker_count} workers failed")
            return False
            
    except Exception as e:
        print(f"[✗] Error running video processing workflow with {worker_count} workers: {e}")
        return False

def cleanup_minio_objects():
    """Clean up MinIO storage objects created during video processing execution."""
    ...
    try:
        ...
        # List of prefixes to clean up (video processing workflow artifacts)
        temp_prefixes = [
            "video-chunks",
            "mainframes", 
            "filtered-frames",
...
def cleanup_kubernetes_resources():
    """Clean up Kubernetes pods and jobs created during video processing execution."""
...
def cleanup_temp_files():
    """Clean up temporary files created during video processing execution."""
    ...
    try:
        ...
        subprocess.run(
            ["find", "/tmp", "-name", "*.mp4", "-type", "f", "-delete"],
            capture_output=True
        )
        ...
        subprocess.run(
            ["find", "/tmp", "-name", "*.json", "-type", "f", "-delete"],
            capture_output=True
        )
...
def run_single_worker_configuration(worker_count):
    """Run video processing workflow with a specific worker configuration and clean up afterwards."""
    print(f"\n{'='*80}")
    print(f"EXECUTING WORKER CONFIGURATION: {worker_count} WORKERS")
    print(f"{'='*80}")
    
    # Step 1: Run the video processing workflow with specific worker count
    workflow_success = run_video_workflow_with_workers(worker_count)
...
def main():
    ...
    # Define worker configurations to test for video processing
    worker_configurations = [1, 2, 4, 8, 12, 16, 20, 24]
    
    print("="*100)
    print("BATCH VIDEO PROCESSING WORKFLOW EXECUTION WITH CLEANUP")
    print("SEQUENTIAL EXECUTION WITH WORKER CONFIGURATIONS")
    print("="*100)
    ...
    # Print final summary
    print("\n" + "="*100)
    print("BATCH EXECUTION FINAL SUMMARY - VIDEO PROCESSING")
    print("="*100)
